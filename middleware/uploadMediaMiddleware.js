...
const { maxImagesProduct, maxImageSize, maxVideoSize, mimeTypesImages, mimeTypesVideos, maxVideosProduct } = config;
...
const validateFiles = (files, maxCount, maxSize, mimeTypes, fileType) => {
  ...
  const invalidFile = files.find(file => file.size > maxSize || !mimeTypes.hasOwnProperty(file.mimetype));
  if (invalidFile) {
    if (invalidFile.size > maxSize) {
      return `Размер ${fileType} файла превышает максимально допустимый: ${maxSize/1024/1024} Мб`;
    }
...
const uploadMediaMiddleware = (req, res, next) => {
  if (!req.files?.img && !req.files?.video) {
    return next(BadRequestError("Не загружено ни одного изображения или видео"));
  }
  ...
  if (req.files?.img) {
    ...
    errorMessage = validateFiles(images, maxImagesProduct, maxImageSize, mimeTypesImages, "изображения");
  }

  if (!errorMessage && req.files?.video) {
    const videos = normalizeFileArray(req.files.video);
    errorMessage = validateFiles(videos, maxVideosProduct, maxVideoSize, mimeTypesVideos, "видео");
  }
